...
export function BookingForm({ vehicle, locations, hasActivePromos = false, user, reservedDates = [] }: BookingFormProps) {
  ...
  const handleDownload = async () => {
    ...
    doc.setTextColor(0, 242, 254); doc.setFontSize(22); doc.setFont('helvetica', 'bolditalic'); doc.text('DUBAI RENT CARS', 20, 30)
    doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(150, 150, 150); doc.text('PREMIUM FLEET MATRIX - OFFICIAL REQUEST', 20, 38)
    doc.setDrawColor(0, 242, 254); doc.setLineWidth(0.5); doc.line(20, 45, 190, 45)

    doc.setTextColor(255, 255, 255); doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text(t('clientInfo'), 20, 60)
    doc.setFontSize(11); doc.setFont('helvetica', 'normal'); doc.text(`Name: ${bookingData?.clientName}`, 25, 70)
    doc.text(`Email: ${bookingData?.clientEmail}`, 25, 78); doc.text(`Phone: ${bookingData?.clientPhone}`, 25, 86)
    doc.text(`ID/Passport: ${bookingData?.idNumber}`, 25, 94)

    doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text('VEHICLE CONFIGURATION', 20, 110)
    doc.setFontSize(11); doc.setFont('helvetica', 'normal'); doc.text(`${vehicle.brand} ${vehicle.name}`, 25, 120)
    doc.text(`Power: ${vehicle.specs?.power}`, 25, 128); doc.text(`Top Speed: ${vehicle.specs?.topSpeed}`, 25, 136)
    doc.text(`Model Year: ${vehicle.modelYear}`, 25, 144)

    doc.setFontSize(14); doc.setFont('helvetica', 'bold'); doc.text(t('duration'), 20, 160)
    doc.setFontSize(11); doc.setFont('helvetica', 'normal'); doc.text(`Start: ${new Date(bookingData?.startDate).toLocaleString()}`, 25, 170)
    doc.text(`End: ${new Date(bookingData?.endDate).toLocaleString()}`, 25, 178)
    doc.text(`Pickup: ${bookingData?.pickupLocation}`, 25, 186); doc.text(`Return: ${bookingData?.returnLocation}`, 25, 194)

    doc.setFillColor(30, 30, 30); doc.rect(20, 210, 170, 30, 'F')
    doc.setTextColor(0, 242, 254); doc.setFontSize(16); doc.setFont('helvetica', 'bold'); doc.text(t('total'), 110, 228); doc.text(`${bookingData?.totalPrice} AED`, 150, 228)

    doc.setTextColor(255, 255, 255); doc.setFontSize(10); doc.setFont('helvetica', 'italic'); doc.text(t('visitInstruction'), 20, 260, { maxWidth: 170 })
    doc.setFont('helvetica', 'bold'); doc.text('ID VERIFICATION: PASSED', 20, 280); doc.text(`${t('bookTitle').toUpperCase()} ID: ${bookingData?._id}`, 110, 280)
      ...
      <div className="glass-panel p-16 rounded-[4rem] bg-card/60 border-white/10 text-center space-y-12 animate-fade-in print:p-0 print:border-none print:shadow-none">
        ...
        <div className="flex flex-col items-center gap-6">
           ...
           <div>
              ...
              <p className="text-muted-foreground opacity-60 uppercase font-black text-[10px] tracking-[0.4em] mt-4">Transaction Hub ID: {bookingData?._id?.toUpperCase()}</p>
           </div>
        </div>
        ...
        <div id="booking-request-card" className="p-12 rounded-[2.5rem] bg-white/5 border border-white/5 text-left space-y-10 relative overflow-hidden">
           <div className="flex justify-between items-start border-b border-white/5 pb-8 relative z-10">
              ...
              <div className="text-right">
                 ...
                 <span className="text-4xl font-black text-primary italic">{bookingData?.totalPrice} AED</span>
              </div>
           </div>
      ...
      <div className="glass-panel p-6 sm:p-8 rounded-[2.5rem] bg-card/60 border-white/10 space-y-6 md:space-y-8 relative overflow-hidden">
        ...
        <div className="space-y-3">
           ...
           <h3 className="text-3xl font-black uppercase italic tracking-tighter leading-none">
              {t('bookTitle')} <span className="text-primary italic">Matrix</span>
           </h3>
        </div>
         ...
         <div className="glass-panel p-12 rounded-[3.5rem] bg-card/60 border-white/10 space-y-10 relative overflow-hidden group">
            ...
            <div className="grid grid-cols-2 gap-6">
               {[
                 ...
                 { l: 'Price Per Cycle', v: `${vehicle.pricePerDay} AED`, i: <Gauge className="w-4 h-4" /> },
               ].map((spec, i) => (
            ...
            <div className="p-8 rounded-3xl bg-primary/20 border border-primary/30 flex items-center justify-between">
               ...
               <div className="text-right">
                  {promoResult && <div className="text-sm font-black text-muted-foreground line-through italic opacity-60">{basePrice} AED</div>}
                  <div className="text-3xl font-black text-primary italic">{totalPrice} AED</div>
               </div>
            </div>
         </div>
      </div>
